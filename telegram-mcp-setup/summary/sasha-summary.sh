...
2. Собери деловую сводку по этим лидам:
   ...
   • Разбивка по культурам: культура — суммарный объём в тоннах И в скобках число лидов по этой культуре. Формат строки: "Ячмень — 23 000 т (19)". Сортируй по объёму, по убыванию.
   • Итого тонн за день.
   • 1–3 коротких инсайта (заметные всплески, крупные лиды, необычное, повторяющиеся запросы).
   • Отдельно подсвети аномалии/outliers (тестовые лиды, дубли, подозрительно крупные объёмы) — их исключай из итоговых цифр и помечай для проверки.
